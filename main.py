...
def humans_actions(humans, zombies):
    for human, stats in humans.items():
        pos = stats['position']
        song_zone = [[pos[0], pos[1]], [pos[0], pos[1]]]
        speed = stats['speed']
        vision = stats['vision']
        for x, zombie in enumerate(zombies):
            ...
            if abs(coord[0]) <= vision and abs(coord[1]) <= vision:
                ...
                song_zone = [
                    [song_zone[0][0] - song_area, song_zone[0][1] - song_area],
                    [song_zone[1][0] + song_area, song_zone[1][1] + song_area],
                ]
        ...
        song_zone = [
            [song_zone[0][0] - 1.5*speed, song_zone[0][1] - 1.5*speed],
            [song_zone[1][0] + 1.5*speed, song_zone[1][1] + 1.5*speed],
        ]
        humans[human]['song_zone'] = song_zone
    ...
    for x, zombie in enumerate(zombies):
        ...
        for human, stats in hum.items():
            ...
            if abs(coord[0]) <= 1 and abs(coord[1]) <= 1:
                ...
                humans.pop(human)
            elif song_zone[0][0] <= zombie[0] <= song_zone[1][0] and song_zone[0][1] <= zombie[1] <= song_zone[1][1]:
                zombies[x] = [
    ...
    for i in range((nb*3)//5):
        ...
        for gen in death_list:
            ...
            if rand >= last_prob and rand < prob + last_prob:
                ...
                break
            last_prob += prob
    humans |= init_humans(humans, (nb*2)//5)
    return humans
...
for i in range(nb_gen):
    ...
    death_list = []
    k = 0
    while len(humans) > 0 and len(zombies) > 0 and k < 2*(world_area + nb):
        humans, zombies, death_list = actions(humans, zombies, death_list)
        k += 1

    if len(humans) > 0:
        winners = f'\rHumans won, {len(humans)} were still alive'
        death_list = kill_human(humans, death_list)
    else: winners = f'\rZombies won, {len(zombies)} were still alive'

    humans = create_new_gen(death_list)
    print(winners, ' :\t',progress_bar(i+1, nb_gen), end='\r')
print('\n')
